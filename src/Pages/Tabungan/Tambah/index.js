...
export default function TambahTabungan() {
  ...
  const onFirstInit = async () => {
    try {
      const response = await Request.FGetWA(Url.API.TABUNGAN.MAXIMUM);
      if (response.success) {
  ...
  const onTopup = async () => {
    try {
      setLoading(() => true);
      const response = await Request.FPostWA(Url.API.TABUNGAN.TOPUP, {
      ...
      if (response.success) {
        ...
        navigation.replace('app.tabungan.detail', {
          id: response.data.id,
        });
      ...
      <ScrollView>
        ...
        <View style={styles.container}>
          <View style={styles.header}>
            <RenderHTML
              source={{ html: infoMax['deskripsi-investasi'] }}
            ...
            <Text style={[styles.textHeader, {
              fontSize: scaleWidth(2.6),
              color: 'white',
            }]}>
              "Dan sempurnakan ibadah haji & umrah karena Allah " (QS. Al Baqarah:196) {'\n'}
            </Text>
            <Text style={styles.textHeader}>
              Berapa banyak yang ingin anda simpan ?
            </Text>
          </View>
          <View style={styles.formGroup}>
            ...
            <View style={styles.inputField}>
              <View style={{ flexDirection: 'row' }}>
                {infoMax.tabungan &&
                  infoMax.tabungan.map(item => {
                    return (
                      <TouchableOpacity
                        key={item}
                        onPress={() => setAmount(item)}
                        style={{
                          marginEnd: scaleWidth(2),
          ...
          <View style={styles.footer}>
            ...
            <TouchableOpacity
              ...
              <Text style={styles.btnSendLabel}>Tabung</Text>
            </TouchableOpacity>
          </View>
        </View>
      </ScrollView>

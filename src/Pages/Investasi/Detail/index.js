...
export default function InvestasiDetail() {
  ...
  const onRefusedTransaction = async () => {
    ...
    try {
      const response = await Request.FPostWA(
        Url.API.INVESTASI.CANCEL + '?id=' + id,
      );
  ...
  const getDetailTransaction = async () => {
    try {
      const response = await Request.FGetWA(
        Url.API.INVESTASI.DETAIL + '?id=' + id,
      );

      console.log('DETAIL INVESTASI', response.data);
